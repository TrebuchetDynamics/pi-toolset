...
Use per-profile `SOUL.md` for stable identity/remit and the actual approved repository mission, not just generic coordinator duties. Apply the [root briefing contract](root-briefing.md): absolute briefing pointer, source-checked first-contact onboarding and conversational readiness on the real root surface. Put shared repository conventions in the existing effective project context; do not add `.hermes.md` that accidentally shadows `AGENTS.md`. Keep transient task details on cards. Installed skills expose metadata, not necessarily full procedures: record the loading mechanism and actual installed names. Task `skills` can preload supported installed skills; they do not install them. Project-local skills may need trust, including in new worktrees. Do not invent `kanban-orchestrator`/`kanban-worker` skills when lifecycle guidance is injected natively.
