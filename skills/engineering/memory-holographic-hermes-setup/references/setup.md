...
For a **hermes-repo-install Compose handoff**, select the container-target adapter below **before** launcher or profile discovery. Every mention of Hermes, Python, installed source, config or dependencies then means that container's runtime, not the host. A host without Hermes, Python or `~/.hermes` is valid. Before the approved container exists, mark runtime checks pending and return to repo provisioning; do not install host Hermes to unblock them.

Read-only discovery comes first. Locate the Hermes launcher and its installation/venv from the actual executable in the selected runtime; do not assume a global `python` or a remembered checkout path. Read installed source before imports or commands that could initialize state, install plugins, or contact services. Record Hermes version and Holographic source/revision when available. Check CLI help for supported config and memory commands.
...
The repo installer owns the single-writer maintenance window and deployment lifecycle; reuse its explicit approval for local configuration/canary and first startup rather than asking for them again. Its host `.hermes/.env` holds installer-managed bootstrap/web secrets, injected by Compose. The user privately runs `hermes setup`; native provider/channel credentials and OAuth state remain inside that instance's persistent home under the installer's host-CLI contract. Memory setup must not read/copy those secrets, create credential files or run the wizard. On a fresh install, run this memory handoff after the user finishes setup and the installer rechecks quiescence and the repo workspace `/workspace`, because the wizard may change memory/workspace settings. Do not start/stop a second gateway or maintenance container implicitly. Confirm s6/cron/dashboard writers are actually quiescent, not just that a CMD changed. Never execute the probe using host Python. If the image's `/opt/hermes` install is immutable, dependency changes must use an approved supported persistent plugin environment or a reproducible derived image—not root `pip install` into the running venv. No provider fallback if Holographic is the installer's required default. Report local verification separately from gateway readiness.
