...
Offer persistence once: “Install these commands in `~/.local/bin` for new terminals and scripts?” Name the selected entries: **core** (`hermes-<repo-name>`, `-status`, `-logs`) during setup, or all four only after apply's gates pass. Reuse explicit acceptance/decline and installed mode. Acceptance authorizes only the shown entries—not PATH edits, shell-rc migration or replacing another tool. Check the selected names against the user's actual shell, existing aliases/functions and earlier PATH entries; a subprocess's PATH alone does not prove that user's shell resolution. Preserve existing shell-only persistence unless a migration was requested. Never create both persistence modes automatically.

After ownership/identity checks and acceptance, invoke the skill-relative [shortcut installer](../scripts/install-shortcuts.mjs) through Node with absolute paths. Before apply is verified, install the core commands explicitly:

```sh
node '<skill-directory>/scripts/install-shortcuts.mjs' --repo '<canonical-repo>' --bin-dir '<user-home>/.local/bin' --core-only
```

`--core-only` selects exactly main/status/logs. It neither inspects nor changes an existing source/destination `-apply` entry, and never removes previously installed links. Do not infer that an unselected apply command is absent, safe or installed by this run. Missing or unsafe **core** launchers still fail the whole selected-set preflight; there is no automatic partial selection.

After setup/workspace/memory and image-specific probe gates pass, generate the verified apply launcher and rerun the same command **without `--core-only`**. The default requires all four launchers; identical core links are preserved and only the missing apply link is added. Verify all selected names and actual shell resolution again. Reuse prior exact-scope approval if it included the fourth entry; otherwise ask only to add `-apply`, not to reinstall the core set. Neither creating the launcher nor linking it executes apply or authorizes a restart.

The helper verifies owned executable repo-local targets and POSIX replacement boundaries along **all** source/destination ancestors—not just private leaf directories. It rejects foreign-owned, symlinked or non-sticky group/world-writable ancestors and destination conflicts; trusted-owner sticky ancestors such as `/tmp` are allowed when the protected descendant chain is owned. It preflights every selected entry before creating absolute symlinks without overwriting anything. An identical existing link is a no-op. A race can leave a partial set; report it, preserve all files and reconcile on rerun rather than removing unverified entries. The helper does not check live Docker ownership or detect aliases in another shell—those remain installer prerequisites. It does not write rc files or change PATH.
...
Record the decision in existing `aliasPersistence` plus optional `shortcutMode: "path" | "shell"`, `shortcutDir` and `shortcutNames` (the verified installed names) fields; record `installed` only after selected link targets and command resolution are verified. Core-only can be `installed` while apply is separately pending; retain the verified core inventory if later apply installation fails. These fields remember progress, not permission. Do not repeat declined offers on resume. A moved/removed repo leaves links that must be diagnosed explicitly; never silently repoint them to another checkout or delete another tool's command.
...
**Missing apply does not block core persistence:** use `--core-only` for the three verified main/status/logs launchers. Without this explicit flag the helper still requires all four; never fabricate an apply launcher/probe or hand-create links around failed checks. The core-only flag does **not** bypass writable-ancestor checks: option (b) still requires the approved permission repair first. Add a new `-apply` alias/link only after its readiness gates pass; do not advertise an unverified existing apply entry or delete it implicitly.

For an initial accepted but wholly blocked PATH attempt, retain `aliasPersistence: "requested"`, `shortcutMode: "path"` and the intended `shortcutDir`; do not invent `aliasPersistence: "blocked"`, reset it to `not-offered`, or claim `installed`. During authorized setup, record the observed paths/modes, created entries and bounded blocker in nonsecret evidence using the [resume contract](resume-and-diagnostics.md). Preserve runtime phase and any earlier incomplete gate; an optional shortcut blocker is not a gateway diagnosis. On resume, reuse the decision and evidence; retry only after the relevant inputs change.
