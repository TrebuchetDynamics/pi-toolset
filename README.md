...
No host Hermes installation, CLI, profile or Python environment is required: Hermes and its memory checks run inside the selected Docker container. The workflow reuses repo-local provider/model choices, keeps web off unless requested, and groups deployment scope into one concise approval rather than reconfirming defaults. Dirty source is preserved.

All setup-managed secrets use one ignored, owner-only `<repo>/.hermes/.env` (`0600`), injected through a raw Compose `env_file` in both headless and web modes (Compose 2.30+). External keys are entered privately; missing local web-auth secrets are generated without printing them and reused on reruns. No separate `web.env`, embedded YAML credentials or automatic host-credential copying. Existing alternate secret stores require an explicit consolidation plan; OAuth flows that require token files outside `.env` need a compatible authentication choice. Environment injection is not a vault—Docker administrators and container processes can access values.
