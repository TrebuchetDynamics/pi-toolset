...
Default engineering entry points are Superpowers `brainstorming`, `systematic-debugging`, and `test-driven-development`. The skills below are included in the installer's `all` profile (or their narrower profile); command bridges may additionally require an enabled extension.
...
The automation skill [`memory-holographic-hermes-setup`](skills/engineering/memory-holographic-hermes-setup/SKILL.md) configures memory for selected **existing** Hermes profiles without creating a team or changing models, credentials, SOUL, or gateways. It checks installed provider support, profile/database isolation and NumPy in the Hermes Python environment; preserves existing state; and verifies local persistence with a temporary fact and cleanup. Provider switches, dependency installation, database relocation and service restarts require specific approval. `auto_extract: false` is the default, not a guarantee that the agent cannot save facts.

For catalog discovery and `/skill:` commands, use `sh install-agent-skills.sh --profile=all` to include every maintained skill, or `--profile=automation` for the narrower core-plus-automation selection. Both refresh the global skill bundle and record the profile without configuring Hermes. Then `/reload` and invoke:

```text
/skill:memory-holographic-hermes-setup atlas beacon
```

The package also declares the prompt shortcut `/memory-holographic-hermes-setup atlas beacon`. It reads the full skill and reference from the catalog or verified package source; readable source-only instructions do not require skill installation or a reload. Genuinely missing files block setup rather than permitting an improvised recipe. Enable the prompt in `pi config` if existing package filters exclude it, then `/reload`. Direct file reading neither registers `/skill:` commands nor changes resource filters. The standalone skill installer does not install Pi prompt templates. Omitting profile names asks for a selection; it never means “all profiles” or the default home.
...
The automation skill [`hermes-repo-install`](skills/engineering/hermes-repo-install/SKILL.md) prepares one isolated Hermes instance per repo and requires `memory-holographic-hermes-setup` for its default memory. Run `/skill:hermes-repo-install`, or `/hermes-repo-install` when the package prompt is enabled, from inside the repo: the target defaults to the Git worktree root resolved from Pi's current working directory. An explicit repository path is an optional override. The shortcut or an explicit source-file invocation can read both skills directly from the verified package checkout when absent from the catalog; it does not require reinstalling skills or reloading Pi just to proceed with discovery and planning. Install/refresh the automation skills and `/reload` as above when catalog commands are desired; enable filtered prompts through `pi config`.

No host Hermes installation, CLI, profile or Python is required. The installer prepares infrastructure, then **you run `hermes setup` in your own terminal** to choose provider/model/channels and enter secrets privately. It never captures the wizard or asks you to paste its output. Existing valid configuration is reused; web stays off unless requested and dirty source is preserved.

Installer-managed bootstrap/web secrets use ignored `<repo>/.hermes/.env` (`0600`) through raw Compose `env_file` (Compose 2.30+). User-run setup owns native provider/channel keys and OAuth state inside persistent `/opt/data`, typically `.env` and `auth.json`. No credential copying, duplicate key authorities, secret-bearing YAML or host-credential borrowing; existing conflicting sources require a specific decision. Docker administrators and runtime processes can still access secrets.

The [host CLI guide](skills/engineering/hermes-repo-install/references/host-cli.md) provides repo-local launchers and short aliases. Source `.hermes/aliases.sh` and run the generated `hermes-<repo-name> setup` alias privately; the repo workspace is already preconfigured as `/workspace` and is verified again afterwards. After initial workspace/memory verification, `hermes-<repo-name>-apply` recreates only that container, preserves its volume, refreshes injected environment variables, then waits up to 60 seconds for a verified image-specific gateway/channel readiness probe. Timeout or unsupported verification returns a nonzero result, not a false success. A plain Docker restart does not refresh Compose-injected env. `hermes-<repo-name>-status` shows scoped container state; `-logs` shows secret-safe historical event summaries, not proof of live readiness. Optional [cross-shell shortcuts](skills/engineering/hermes-repo-install/references/readiness-and-shortcuts.md) install the same four command names in `~/.local/bin` without overwriting existing commands. No `source` is needed when PATH resolution is verified. Persistence is offered once; existing shell-alias mode is preserved and PATH/rc changes require their own explicit scope. A [nonsecret setup receipt](skills/engineering/hermes-repo-install/references/resume-and-diagnostics.md) lets the installer resume the first incomplete gate without repeating completed setup or user decisions; live readiness is always rechecked.

Containers must be named `hermes-<repo-name>` using the full normalized repo basename, such as `hermes-kenworth-cummins-ing`, without hashes or replica suffixes. A name collision blocks setup rather than selecting a different name. Existing container names are preserved until a rename is requested. The repo basename stays the human-facing profile name; canonical-path hashes still isolate Compose projects, volumes and networks. Each owns persistent state at `/opt/data` and mounts the **repo root itself** at `/workspace`, the container and Hermes tool working directory (rechecked after the setup wizard). The default gateway publishes no ports; requested dashboard/API access uses authenticated services with Docker-allocated **loopback-only** host ports, reported after startup and rediscovered after recreation. No shared host `~/.hermes`, Docker socket or host networking.

Its [offline planner](skills/engineering/hermes-repo-install/scripts/compose-plan.mjs) prints a digest-pinned Compose plan without creating files or calling Docker. The [procedure](skills/engineering/hermes-repo-install/references/compose.md) requires ownership checks, request-scoped authorization, secret-safe setup, Holographic verification inside the selected container, and real gateway readiness checks. An explicit install request covers ordinary setup without a second blanket approval; missing inputs or uncovered risks remain blockers. Missing Holographic support blocks readiness rather than silently selecting another provider. Creating/installing the skill does not deploy a container; published tests do not prove live Docker/Hermes readiness.

Existing engineering, research, and optional Understand workflows remain available. No additional workflow plugin, graph service, broad catalog, or scientific toolchain is needed without a specific task. The automation skill [`autonomous-codebase-improver`](skills/engineering/autonomous-codebase-improver/SKILL.md) treats bare/broad requests as continuous, multi-area campaigns: the parent selects concrete approved work, uses one pi-subagents worker and fresh read-only review per slice, accepts the evidence, then continues. Explicit one-slice, collection/subsystem, audit-only, pause/stop, and budget boundaries still apply; missing required delegation blocks implementation. New designs remain approval-gated, and ownership/recoverable slice baselines protect dirty work. Its [evaluation receipt](skills/engineering/autonomous-codebase-improver/references/evaluation-receipt.md) records decision probes, independent review, and parent-guided two-slice, one-slice, timeout/paused-continuation, rollback/refusal, and regression-repair fixtures. Full behavioral acceptance remains provisional: approval/credential branch continuation, unavailable-role/budget, and other lifecycle/type/race variants remain decision-only or unrun; these samples do not prove autonomous reliability. No measured skill-on/skill-off improvement or global activation is claimed.
...
Installing only the Pi package does not install these external tools. The explicit all-in-one `install.sh` command installs OmniRoute and, on fresh installs, all maintained global skills by default; RTK is opt-in; use the commands below for individual setup.
...
The installer installs or refreshes OmniRoute globally with strict Node engine checks and npm's legacy peer resolver (avoiding upstream React/Marked peer-warning noise), restarts the local daemon so provider/model rotation fixes take effect, binds it to `127.0.0.1`, enables crash recovery and autostart, and registers the keyless `auto/best-free` pool so 401, 429, and 504 failures can fall through to another free model when you choose that route. It preserves existing Pi providers/settings, reuses a working endpoint key or creates one for a local install, writes permission-restricted backups, and leaves Pi's default provider/model unchanged—including absent defaults—on installs and updates. It defaults Pi's retry delay to five seconds so OmniRoute's transient model lockout can clear (an existing retry policy is left unchanged). It also persists capacity for eight structurally heavy Pi chats across restarts; lower `OMNIROUTE_CHAT_MAX_HEAVY_IN_FLIGHT` on memory-constrained hosts. Use `--model ID` to register a different advertised model or route; this never selects Pi's default. To use OmniRoute explicitly, run `pi --provider omniroute --model auto/best-free` (or your registered route).
...
Fresh installs use `all`: 15 upstream Superpowers symlinks plus all maintained local skills, including every group in [profiles.json](skills/shared/profiles.json), in `~/.agents/skills` and `~/.claude/skills`. This does not enable optional extensions, configure Hermes or install the tools mentioned by skills. Retired sources stay excluded; manual-only skills retain their metadata. Replaced and deactivated copies are backed up under `~/.local/state/pi-toolset/skill-backups/`; unrelated user skills are preserved.

The standalone skill installer keeps the saved selection. To expand a core-only or narrower install without running the full setup, run:

```bash
sh install-agent-skills.sh --profile=all  # skills only; records all for future updates
# Then /reload in Pi
```
...
`pi update git:github.com/TrebuchetDynamics/pi-toolset` refreshes the package checkout. The skills Pi loads come from the global copies, so refresh those too; `install.sh` does both and defaults to all maintained skills, regardless of the recorded profile. Pass an explicit `--profile=NAME` to keep a narrower selection, including with `/update-pi-toolset`. In an open session, `/update-pi-toolset [--profile=NAME] [--dry-run]` runs that installer for you, then asks you to `/reload`:

```bash
sh install.sh                    # update package + all maintained global skills
sh install.sh --profile=all       # explicitly select the same all-skills default
sh install.sh --profile=design    # explicitly select core + design for this update
